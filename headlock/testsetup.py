...
class TestSetup(BuildInDefs):
    ...
    def __build__(self):
        ...
        def update_cmakelist():
            ...
            src_files_str = ''.join(
                '\n    ' + cmakelist_rel_path(str(abs_src_file))
                for abs_src_file in sorted(abs_src_files)) \
                .replace(os.path.sep, '/')
            cmakelist.set('add_library', key_param=ts_name,
                          params='SHARED' + src_files_str)

            predef_macros = predef_macros
            defines_str = ''.join(
            ...
            incl_dirs_str = ''.join(
                '\n    ' + cmakelist_rel_path(str(abs_incl_dir))
                for abs_incl_dir in sorted(abs_incl_dirs)) \
                .replace(os.path.sep, '/')
